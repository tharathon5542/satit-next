...
class ProfileFaculty extends CI_Controller
{
    ...
    */
    ...
    | --------------------------------------------------------------------------------
    */
    public function onEditProfileFaculty()
    {
        ...
        // validate profileEventColor
        $this->form_validation->set_rules('profileEventColor', '', 'trim');
        ...
        $website =  $this->input->post('profileWebsite', true) ? preg_replace("/[^a-zA-Z0-9_\x{0E00}-\x{0E7F}]/u", '-', $this->input->post('profileWebsite', true))  : null;
        $Link = $this->input->post('profileLink', true) ? $this->input->post('profileLink', true) : null;
        $EventColor = $this->input->post('profileEventColor', true) ? $this->input->post('profileEventColor', true) : null;

        // check website duplicate
        if ($this->db->get_where('cwie_faculty', array('faculty_website' => $website, 'faculty_website !=' => null, 'faculty_id !=' => $id))->num_rows() > 0) {
        ...
        $newProfileData = [
            ...
            'faculty_name_en' => $nameEN,
            'faculty_tel' => $tel,
            'faculty_email' => $email,
            'faculty_website' => $website,
            'faculty_link' => $Link,
            'faculty_event_color' => $EventColor,
        ];
        ...
        if (!$this->db->update('cwie_faculty', $newProfileData, array('faculty_id' => $id))) {
            $response = ['status' => false, 'errorMessage' => 'DB ERROR! : ' . $this->db->error()['message']];
            header('Content-Type: application/json');
            echo json_encode($response);
            return;
        ...
        $crudSessionData['crudWebsite'] = $website;
        $crudSessionData['crudLink'] = $Link;
        $crudSessionData['crudEventColor'] = $EventColor;
        ...
        header('Content-Type: application/json');
        echo json_encode($response = ['status' => true, 'facultyWebsite' => base_url('faculty/' . $website)]);
    }
    ...
    public function onEditFacultyCoordinator()
    {
        ...
        if (!$this->db->update('cwie_faculty', $coordinatorData, array('faculty_id' => $id))) {
            $response = ['status' => false, 'errorMessage' => 'DB ERROR! : ' . $this->db->error()['message']];
            header('Content-Type: application/json');
            echo json_encode($response);
            return;
    ...
    public function onEditFacultyCwiePolicy()
    {
        if (!$this->checkPermission(['faculty'])) {
            $response = ['status' => false, 'errorMessage' => 'Permission Denied!'];
            header('Content-Type: application/json');
        ...
        $facultyCwiePolicyData = [
            'faculty_cwie_policy' => $facultyCwiePolicy,
        ];

        if (!$this->db->update('cwie_faculty', $facultyCwiePolicyData, array('faculty_id' => $id))) {
            $response = ['status' => false, 'errorMessage' => 'DB ERROR! : ' . $this->db->error()['message']];
            header('Content-Type: application/json');
            echo json_encode($response);
            return;
        ...
        $crudSessionData['crudCwiePolicy'] = $facultyCwiePolicy;
