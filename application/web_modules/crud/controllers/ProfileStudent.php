...
class ProfileStudent extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if (!$this->session->userdata('crudSessionData')) {
            ...
            redirect(base_url('crud/auth/students?redirect=') . $redirectURL);
            return;
        }
    }
    ...
    */
    ...
    | --------------------------------------------------------------------------------
    */
    public function onEditProfileStudent()
    {
        ...
        if (!empty($_FILES['profileImage']['name'])) {
            ...
            $queryExistProfileImage = $this->db->get_where('cwie_students_images', array('student_id' => $this->session->userdata('crudSessionData')['crudAuthId']));
            if ($queryExistProfileImage->num_rows() > 0) {
                if (file_exists('./assets/images/profile/' . $queryExistProfileImage->row()->student_image . $queryExistProfileImage->row()->student_image_type)) {
                    // delete exist image
                    if (!unlink('./assets/images/profile/' . $queryExistProfileImage->row()->student_image . $queryExistProfileImage->row()->student_image_type)) {
                ...
                // delete old data
                if (!$this->db->delete('cwie_students_images', array('student_id' => $this->session->userdata('crudSessionData')['crudAuthId']))) {
            ...
            $profileImageData = [
                ...
                'student_image_type' => '.' . $fileExtention[count($fileExtention) - 1],
                'student_id' => $this->session->userdata('crudSessionData')['crudAuthId']
            ];

            // insert profileImageData
            if (!$this->db->insert('cwie_students_images', $profileImageData)) {
                $response = ['status' => false, 'errorMessage' => 'DB ERROR! : ' . $this->db->error()['message']];
                header('Content-Type: application/json');
                echo json_encode($response);
                return;
        ...
        if (!$this->db->update('cwie_students', $newProfileData, array('student_code' => $id))) {
            $response = ['status' => false, 'errorMessage' => 'DB ERROR! : ' . $this->db->error()['message']];
            header('Content-Type: application/json');
            echo json_encode($response);
            return;
